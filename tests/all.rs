...
fn default_font() -> FontT<&'static str> {
    FontT {
        family: "Arial",
        size: 10,
        weight: FONT_WEIGHT_BOLD,
        decoration: FONT_DECO_ITALIC | FONT_DECO_STRIKEOUT,
    }
}
...
macro_rules! test_position {
    ($ui:expr, $id:expr, $t:ty) => {{
        ...
        assert!(
            x.get_position() == (-600, -600),
            "Window position do not match"
        );
    }};
}
...
macro_rules! test_enabled {
    ($ui:expr, $id:expr, $t:ty) => {{
        ...
        assert!(
            x.get_enabled() == false,
            "Window enabled state do not match"
        );
...
fn test_ui_pack_user_value() {
    ...
    let r = ui.commit();
    assert!(
        r.is_err() && r.err().unwrap() == Error::KeyExists,
...
fn test_ui_pack_resource() {
    ...
    assert!(ui.has_id(&1000), "ID 1000 was not found in ui after commit");
    {
        let f = ui.get::<Font>(&1000);
        f.expect("Failed to get control");
    }
...
fn test_ui_unpack() {
    ...
    ui.bind(&1001, &5000, Event::Destroyed, move |_, _, _, _| unsafe {
        *(&mut *x) += 1;
    });
    ui.bind(&1005, &5000, Event::Destroyed, move |_, _, _, _| unsafe {
        *(&mut *x) += 1;
    });
    ...
    // It should be impossible to unpack a borrowed control/resource
    {
        ...
        let r = ui.commit();
        assert!(
            r.is_err() && r.err().unwrap() == Error::ControlInUse,
            "Commit was successful"
        );
        ...
        let r = ui.commit();
        assert!(
            r.is_err() && r.err().unwrap() == Error::ResourceInUse,
            "Commit was successful"
        );
    }
}
...
fn test_ui_bind() {
    ...
    let r = ui.commit();
    assert!(
        r.is_err() && r.err().unwrap() == Error::ControlRequired,
        "Commit was successful"
    );
    ...
    let r = ui.commit();
    assert!(
        r.is_err() && r.err().unwrap() == Error::KeyNotFound,
        "Commit was successful"
    );
    ...
    let r = ui.commit();
    assert!(
        r.is_err() && r.err().unwrap() == Error::EventNotSupported(Event::MouseUp),
        "Commit was successful"
    );
...
fn test_user_trigger() {
    ...
    ui.pack_value(&1001, 5u32);
    ui.pack_control(&1000, window());
    ui.bind(&1000, &5000, Event::MouseDown, move |_, _, _, _| unsafe {
        *(&mut *x) = true;
    });
    ...
    let r = ui.commit();
    assert!(
        r.is_err() && r.err().unwrap() == Error::EventNotSupported(Event::SelectionChanged),
        "Commit was successful"
    );
...
fn test_window_control_user_close() {
    ...
    ui.pack_control(&1000, window());
    ui.bind(&1000, &5000, Event::Destroyed, move |_, _, _, _| unsafe {
        *(&mut *x) = true;
    });
    ui.commit().expect("Commit was not successful");
...
fn test_menus() {
    ...
    ui.pack_control(&1000, window());
    ui.bind(&1000, &10_000, Event::Destroyed, move |_, _, _, _| unsafe {
        *(&mut *x) += 1
    });
    ...
    ui.bind(&1001, &10_000, Event::Destroyed, move |_, _, _, _| unsafe {
        *(&mut *x) += 1
    });
    ...
    ui.bind(&2003, &10_000, Event::Destroyed, move |_, _, _, _| unsafe {
        *(&mut *x) += 1
    });
    ...
    ui.bind(&1002, &10_000, Event::Destroyed, move |_, _, _, _| unsafe {
        *(&mut *x) += 1
    });
    ui.bind(&1003, &10_000, Event::Destroyed, move |_, _, _, _| unsafe {
        *(&mut *x) += 1
    });
    ui.bind(&1004, &10_000, Event::Destroyed, move |_, _, _, _| unsafe {
        *(&mut *x) += 1
    });
    ui.bind(&2000, &10_000, Event::Destroyed, move |_, _, _, _| unsafe {
        *(&mut *x) += 1
    });
    ...
    ui.bind(&1005, &10_000, Event::Destroyed, move |_, _, _, _| unsafe {
        *(&mut *x) += 1
    });
    ui.bind(&1006, &10_000, Event::Destroyed, move |_, _, _, _| unsafe {
        *(&mut *x) += 1
    });
    ui.bind(&1007, &10_000, Event::Destroyed, move |_, _, _, _| unsafe {
        *(&mut *x) += 1
    });
    ui.bind(&2001, &10_000, Event::Destroyed, move |_, _, _, _| unsafe {
        *(&mut *x) += 1
    });
    ...
    ui.bind(&1008, &10_000, Event::Destroyed, move |_, _, _, _| unsafe {
        *(&mut *x) += 1
    });
    ui.bind(&1009, &10_000, Event::Destroyed, move |_, _, _, _| unsafe {
        *(&mut *x) += 1
    });
    ui.bind(&2002, &10_000, Event::Destroyed, move |_, _, _, _| unsafe {
        *(&mut *x) += 1
    });
...

    {
        ...
        let (menu, menuitem) = (
            ui.get::<Menu>(&1001).unwrap(),
            ui.get::<MenuItem>(&2002).unwrap(),
        );
    ...
    ui.commit().expect("Commit was not successful");
    assert!(
        ui.has_id(&1001) == false,
        "Destroyed menu key '1001' was found in ui"
    );
    ...
    ui.commit().expect("Commit was not successful");
    assert!(
        ui.has_id(&1002) == false,
        "Destroyed menu key '1002' was found in ui"
    );
    assert!(
        ui.has_id(&1003) == false,
        "Destroyed menu key '1003' was found in ui"
    );
    assert!(
        ui.has_id(&1004) == false,
        "Destroyed menu key '1004' was found in ui"
    );
    assert!(
        ui.has_id(&2000) == false,
        "Destroyed menu item key '2000' was found in ui"
    );
    ...
    ui.commit().expect("Commit was not successful");
    assert!(
        ui.has_id(&1005) == false,
        "Destroyed menu key '1005' was found in ui"
    );
    assert!(
        ui.has_id(&1006) == false,
        "Destroyed menu key '1006' was found in ui"
    );
    assert!(
        ui.has_id(&1007) == false,
        "Destroyed menu key '1007' was found in ui"
    );
    assert!(
        ui.has_id(&2001) == false,
        "Destroyed menu item key '2001' was found in ui"
    );
    ...
    ui.commit().expect("Commit was not successful");
    assert!(
        ui.has_id(&1000) == false,
        "Destroyed menu key '1000' was found in ui"
    );
    assert!(
        ui.has_id(&1008) == false,
        "Destroyed menu key '1008' was found in ui"
    );
    assert!(
        ui.has_id(&1009) == false,
        "Destroyed menu key '1009' was found in ui"
    );
    assert!(
        ui.has_id(&2002) == false,
        "Destroyed menu item key '2002' was found in ui"
    );
...
fn test_buttons() {
    ...
    match ui.commit() {
        ...
        r => panic!("Should have returned Error::KeyNotFound, got {:?}", r),
    }

    btn_t.parent = 1001;
    ui.pack_control(&1004, btn_t.clone());
    match ui.commit() {
        Err(Error::BadParent(_)) => {}
        r => panic!("Should have returned Error::BadParent, got {:?}", r),
    }
    ...
    ui.pack_control(&1004, btn_t.clone());
    match ui.commit() {
        Err(Error::BadResource(_)) => {}
        r => panic!("Should have returned Error::BadResource, got {:?}", r),
    }
...
fn test_listbox() {
    ...
    {
        let mut lb = ui
            .get_mut::<ListBox<&'static str>>(&1002)
            .expect("Control not found!");

        assert!(lb.get_readonly() == false, "Listbox should not be readonly");
        assert!(
            lb.get_multi_select() == false,
            "Listbox should not be multi select"
        );
        ...
        assert!(
            lb.get_multi_select() == true,
            "Listbox should be multi-select"
        );
        ...
        lb.push("Foohoy!");
        assert!(
            lb.collection() == &["Foo", "FooBar", "Excelsior", "wOOsh", "Foohoy!"],
            "Collection do not match"
        );
        assert!(
            lb.get_string(4).unwrap().as_str() == "Foohoy!",
            "Item text do not match"
        );
        assert!(lb.len() == 5, "Collection length should be 5");

        lb.remove(0);
        assert!(
            lb.collection() == &["FooBar", "Excelsior", "wOOsh", "Foohoy!"],
            "Collection do not match"
        );
        assert!(
            lb.get_string(0).unwrap().as_str() == "FooBar",
            "Item text do not match"
        );

        assert!(
            lb.get_selected_index().is_none(),
            "No index should be selected"
        );
        assert!(
            lb.get_selected_indexes().len() == 0,
            "Indexes vector length should be 0"
        );
        ...
        assert!(
            lb.find_string("foo", false) == Some(0),
            "find_string shoud have returned 0"
        );
        assert!(
            lb.find_string("foo", true) == None,
            "find_string shoud have returned None"
        );
        assert!(
            lb.find_string("Foohoy!", true) == Some(3),
            "find_string shoud have returned None"
        );
        ...
        assert!(
            lb.get_string(0).unwrap().as_str() == "FooBar",
            "Item text do not match"
        ); // Ui and inner collection not synced
        lb.sync();
        assert!(
            lb.get_string(0).unwrap().as_str() == "Excelsior",
            "Item text do not match"
        ); // Ui and inner collection synced
...

    {
        ...
        assert!(
            lb.get_selected_indexes() == [0, 2],
            "Selected indexes do not match"
        );

        lb.set_index_selected(0, false);
        assert!(lb.len_selected() == 1, "Selected length is not 1");
        assert!(
            lb.get_selected_indexes() == [2],
            "Selected indexes do not match"
        );
        ...
        assert!(
            lb.get_selected_indexes() == [0, 1, 2, 3],
            "Selected indexes do not match"
        );
...
fn test_combobox() {
    ...
    {
        let mut cb = ui
            .get_mut::<ComboBox<&'static str>>(&1002)
            .expect("Control not found!");
        ...
        cb.push("Foohoy!");
        assert!(
            cb.collection() == &["Foo", "FooBar", "Excelsior", "wOOsh", "Foohoy!"],
            "Collection do not match"
        );
        assert!(
            cb.get_string(4).unwrap().as_str() == "Foohoy!",
            "Item text do not match"
        );
        ...
        cb.remove(0);
        assert!(
            cb.collection() == &["FooBar", "Excelsior", "wOOsh", "Foohoy!"],
            "Collection do not match"
        );
        assert!(
            cb.get_string(0).unwrap().as_str() == "FooBar",
            "Item text do not match"
        );

        assert!(
            cb.get_selected_index().is_none(),
            "No index should be selected"
        );
        ...
        assert!(
            cb.find_string("foo", false) == Some(0),
            "find_string shoud have returned 0"
        );
        assert!(
            cb.find_string("foo", true) == None,
            "find_string shoud have returned None"
        );
        assert!(
            cb.find_string("Foohoy!", true) == Some(3),
            "find_string shoud have returned None"
        );
        ...
        cb.sync();
        assert!(
            cb.get_string(0).unwrap().as_str() == "Excelsior",
            "Item text do not match"
        ); // Ui and inner collection synced
