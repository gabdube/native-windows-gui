...
use controls::{AnyHandle, Control, ControlT};
use error::Error;
...
impl<ID: Hash + Clone> UiInner<ID> {
    pub fn new() -> Result<UiInner<ID>, Error> {
        use low::other_helper::{enable_com, enable_visual_styles};
    ...
    pub fn pack_user_value(&mut self, params: PackUserValueArgs<ID>) -> Option<Error> {
        ...
        if self.inner_public_map.contains_key(&inner_id) {
            ...
            self.inner_public_map
                .insert(inner_id, (params.id, params.tid));
            self.user_values
                .insert(inner_id, RefCell::new(params.value));
            None
        }
    }
        ...
        if self.inner_public_map.contains_key(&inner_id) {
            ...
            match params.value.build(&tmp_ui) {
                Ok(control) => {
                    ...
                    let mut event_collection: EventCollection<ID> =
                    ...
                    self.inner_public_map
                        .insert(inner_id, (params.id, params.value.resource_type_id()));
    ...
    fn unpack_control(&mut self, id: InnerId) -> Option<Error> {
        ...
        for id in children_ids.iter().rev() {
            ...
            let handle_hash = *self
                ...
                .iter()
                .find(|&(_, ref v)| *v == id)
                .expect("Could not match handle while unpacking control")
                .0;
    ...
    pub fn inner_id_from_handle(&self, handle: &AnyHandle) -> Option<InnerId> {
        if let Some(id) = self
            .handle_inner_map
            .get(&UiInner::<ID>::hash_handle(handle))
        {
            Some(*id)
        } else {
            None
...
impl<ID: Hash + Clone> Ui<ID> {
    ...
    */
    pub fn pack_value<T: Into<Box<T>> + 'static>(&self, id: &ID, value: T) {
        ...
        let data = PackUserValueArgs {
            id: id.clone(),
            tid: TypeId::of::<T>(),
            value: value.into() as Box<Any>,
        };
        inner
            .messages
            .post(self.inner, NWG_PACK_USER_VALUE, Box::new(data) as Box<Any>);
    }
    ...
    */
    pub fn pack_control<T: ControlT<ID> + 'static>(&self, id: &ID, value: T) {
        ...
        let inner = unsafe { &mut *self.inner };
        let data = PackControlArgs {
            id: id.clone(),
            value: Box::new(value),
        };
        inner
            .messages
            .post(self.inner, NWG_PACK_CONTROL, Box::new(data) as Box<Any>);
    }
    ...
    */
    pub fn pack_resource<T: ResourceT<ID> + 'static>(&self, id: &ID, value: T) {
        ...
        let inner = unsafe { &mut *self.inner };
        let data = PackResourceArgs {
            id: id.clone(),
            value: Box::new(value),
        };
        inner
            .messages
            .post(self.inner, NWG_PACK_RESOURCE, Box::new(data) as Box<Any>);
    }
    ...
    */
    pub fn unpack(&self, id: &ID) {
        ...
        inner
            .messages
            .post(self.inner, NWG_UNPACK, Box::new(data) as Box<Any>);
    }
    ...
    */
    pub fn get<T: 'static>(&self, id: &ID) -> Result<Ref<Box<T>>, Error> {
        ...
        if !inner.inner_public_map.contains_key(&inner_id) {
            return Err(Error::KeyNotFound);
        }
        if !inner.types_matches(&inner_id, TypeId::of::<T>()) {
    ...
    */
    pub fn bind<T>(&self, id: &ID, cb_id: &ID, event: Event, cb: T)
    where
        T: Fn(&Ui<ID>, &ID, &Event, &EventArgs) -> () + 'static,
    {
        ...
        inner
            .messages
            .post(self.inner, NWG_BIND, Box::new(data) as Box<Any>);
    }
    ...
    */
    pub fn unbind(&self, id: &ID, cb_id: &ID, event: Event) {
        ...
        inner
            .messages
            .post(self.inner, NWG_UNBIND, Box::new(data) as Box<Any>);
    }
    ...
    */
    pub fn trigger(&self, id: &ID, event: Event, event_arg: EventArgs) {
        ...
        inner
            .messages
            .post(self.inner, NWG_TRIGGER, Box::new(data) as Box<Any>);
    }
