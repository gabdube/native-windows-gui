...
unsafe extern "system" fn process_events<ID: Hash+Clone+'static>(hwnd: HWND, msg: UINT, w: WPARAM, l: LPARAM, id: UINT_PTR, scope: DWORD_PTR) -> LRESULT {
...
pub fn unhook_window_events<ID: Hash+Clone+'static>(handle: HWND) { unsafe {
  ...
  if GetWindowSubclass(handle, Some(process_events::<ID>), EVENTS_DISPATCH_ID, &mut data) == TRUE {
    Box::from_raw(mem::transmute::<_, *mut UiWithID<ID>>(data));
