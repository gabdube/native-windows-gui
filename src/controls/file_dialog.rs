...
use winapi::{IShellItem, FILEOPENDIALOGOPTIONS, HWND, S_OK};
...
impl<S1: Clone + Into<String>, ID: Hash + Clone> ControlT<ID> for FileDialogT<S1, ID> {
    ...
    fn build(&self, ui: &Ui<ID>) -> Result<Box<Control>, Error> {
        use low::clsid::{
            CLSID_FileOpenDialog, CLSID_FileSaveDialog, UUIDOF_IFileDialog, UUIDOF_IFileOpenDialog,
        };
        use low::window_helper::handle_of_window;
        use ole32::CoCreateInstance;
        use winapi::{
            CLSCTX_INPROC_SERVER, FOS_ALLOWMULTISELECT, FOS_FORCEFILESYSTEM, FOS_PICKFOLDERS,
        };

        let parent = match self.parent.as_ref() {
            Some(id) => match handle_of_window(
                ui,
                id,
                "The parent of a FileDialog must be a window-like control.",
            ) {
                Ok(h) => h,
                Err(e) => {
                    return Err(e);
        ...
        if r != S_OK {
            return Err(Error::System(SystemError::ComInstanceCreation(
                "FileDialog".to_string(),
            )));
        }
        ...
        unsafe {
            ...
            if pfd.GetOptions(&mut flags) != S_OK {
                pfd.Release();
                return Err(Error::System(SystemError::ComError(
                    "Failed to get the file dialog options".to_string(),
                )));
            }

            let use_dir = if self.action == FileDialogAction::OpenDirectory {
                FOS_PICKFOLDERS
            } else {
                FILEOPENDIALOGOPTIONS(0)
            };
            let multiselect = if self.multiselect {
            ...
            if pfd.SetOptions(flags | FOS_FORCEFILESYSTEM | use_dir | multiselect) != S_OK {
                pfd.Release();
                return Err(Error::System(SystemError::ComError(
                    "Failed to set the file dialog options".to_string(),
                )));
            }
...
impl FileDialog {
    ...
    */
    pub fn get_selected_item(&self) -> Result<String, Error> {
        unsafe {
            if self.multiselect {
                return Err(Error::UserError(
                    "FileDialog have the multiselect flag".to_string(),
                ));
            }
    ...
    */
    pub fn get_selected_items(&self) -> Result<Vec<String>, Error> {
        unsafe {
            use low::defs::IShellItemArray;
            use winapi::{IFileOpenDialog, DWORD};

            if self.action == FileDialogAction::Save {
                return Err(Error::UserError(
                    "Save dialog cannot have more than one item selected".to_string(),
                ));
            }
            ...
            if handle.GetResults(mem::transmute(&mut _items)) != S_OK {
                return Err(Error::System(SystemError::ComError(
                    "Failed to get results".to_string(),
                )));
            }
    ...
    */
    pub fn set_multiselect(&mut self, multiselect: bool) -> Result<(), Error> {
        ...
        if self.action == FileDialogAction::Save {
            return Err(Error::UserError(
                "Cannot set multiselect flag for a save file dialog".to_string(),
            ));
        }
...
#[inline(always)]
unsafe fn set_default_folder<S: Clone + Into<String>>(
...
#[inline(always)]
unsafe fn set_filters<S: Clone + Into<String>>(
    dialog: &mut IFileDialog,
    filters: &S,
) -> Result<(), Error> {
...
#[inline(always)]
unsafe fn toggle_dialog_flags(
    dialog: &mut IFileDialog,
    flag: FILEOPENDIALOGOPTIONS,
    enabled: bool,
) -> Result<(), Error> {
    let mut flags: FILEOPENDIALOGOPTIONS = FILEOPENDIALOGOPTIONS(0);
