...
use winapi::{DATETIMEPICKERINFO, HFONT, HWND, SYSTEMTIME};

use controls::{AnyHandle, Control, ControlT, ControlType};
use defs::{CheckState, HTextAlign, PickerDate};
use error::Error;
use events::Event;
...
impl<S: Clone + Into<String>, ID: Hash + Clone> ControlT<ID> for DatePickerT<S, ID> {
    ...
    fn build(&self, ui: &Ui<ID>) -> Result<Box<Control>, Error> {
        ...
        use winapi::{DTS_RIGHTALIGN, DTS_SHOWNONE, DWORD, WS_CHILD, WS_DISABLED, WS_VISIBLE};
            ...
            | match self.align {
                HTextAlign::Center => {
                    return Err(Error::UserError(
                        "The date time picker control do not support centered text.".to_string(),
                    ));
                }
                HTextAlign::Left => 0,
        ...
        let parent = match handle_of_window(
            ui,
            &self.parent,
            "The parent of a date time picker must be a window-like control.",
        ) {
            Ok(h) => h,
            Err(e) => {
                return Err(e);
        ...
        let font_handle: Option<HFONT> = match self.font.as_ref() {
            Some(font_id) => match handle_of_font(
                ui,
                &font_id,
                "The font of a date time picker must be a font resource.",
            ) {
                Ok(h) => Some(h),
                Err(e) => {
                    return Err(e);
...
impl DatePicker {
    ...
    */
    pub fn get_value(&self) -> Option<PickerDate> {
        ...
        let r =
            unsafe { SendMessageW(self.handle, DTM_GETSYSTEMTIME, 0, mem::transmute(&mut syst)) };
        match r {
    ...
    */
    pub fn set_value(&self, date: &Option<PickerDate>) {
        use winapi::{DTM_SETSYSTEMTIME, GDT_NONE, GDT_VALID, WPARAM};
        unsafe {
            match date {
                &Some(ref date) => {
    ...
    */
    pub fn get_align(&self) -> HTextAlign {
        ...
        use winapi::{DTS_RIGHTALIGN, GWL_STYLE};
    ...
    */
    ...
    pub fn close_calendar(&self) {
        ...
        unsafe {
            SendMessageW(self.handle, DTM_CLOSEMONTHCAL, 0, 0);
        }
    }
