...
use winapi::{
    ID2D1Brush, ID2D1StrokeStyle, D2D1_ELLIPSE, D2D1_MATRIX_3X2_F, D2D1_POINT_2F, D2D1_RECT_F,
    D2D1_ROUNDED_RECT, FLOAT,
};

use super::{Canvas, CanvasProtected, CanvasResources};
use defs::{Ellipse, Rectangle};
use error::Error;
...
impl<'a, ID: Clone + Hash> CanvasRenderer<'a, ID> {
    ...
    */
    pub fn set_transform(&mut self, m: &[[FLOAT; 2]; 3]) {
        unsafe {
            self.SetTransform(&D2D1_MATRIX_3X2_F { matrix: *m });
        }
    }
    ...
    */
    pub fn fill_rounded_rectangle(
        ...
        unsafe {
            self.FillRoundedRectangle(&rect, mem::transmute(brush));
        }
        Ok(())
    }
    ...
    */
    pub fn fill_ellipse(&mut self, brush: &ID, e: &Ellipse) -> Result<(), Error> {
        ...
        unsafe {
            self.FillEllipse(&ellipse, mem::transmute(brush));
        }
        Ok(())
    }
    ...
    */
    pub fn draw_rectangle(
        ...
        unsafe {
            self.DrawRectangle(&rect, mem::transmute(brush), width, mem::transmute(pen));
        }
    ...
    */
    pub fn draw_rounded_rectangle(
        ...
        unsafe {
            self.DrawRoundedRectangle(&rect, mem::transmute(brush), width, mem::transmute(pen));
        }
    ...
    */
    pub fn draw_ellipse(
        ...
        unsafe {
            self.DrawEllipse(&ellipse, mem::transmute(brush), width, mem::transmute(pen));
        }
    ...
    fn fill_setup(&mut self, brush: &ID) -> Result<*mut ID2D1Brush, Error> {
        match self.get_resource(brush) {
            Ok(brush) => match brush {
                ...
                CanvasResources::StrokeStyle(_) => Err(Error::BadResource(
                    "Resource of type brush required, got Brush.".to_string(),
                )),
            },
            Err(e) => Err(e),
        }
    ...
    fn draw_setup(
        ...
        brush: &ID,
        pen: Option<&ID>,
    ) -> Result<(*mut ID2D1Brush, *mut ID2D1StrokeStyle), Error> {
        let brush = match self.get_resource(brush) {
            Ok(brush) => match brush {
                CanvasResources::SolidBrush(b) => b,
                CanvasResources::StrokeStyle(_) => {
                    return Err(Error::BadResource(
                        "Resource of type brush required, got Pen.".to_string(),
                    ));
                }
            },
            Err(e) => {
        ...
        let pen = match pen {
            Some(pen) => match self.get_resource(pen) {
                Ok(pen) => match pen {
                    ...
                    CanvasResources::SolidBrush(_) => {
                        return Err(Error::BadResource(
                            "Resource of type pen required, got Brush.".to_string(),
                        ));
                    }
                },
                Err(e) => {
...
impl<'a, ID: Clone + Hash> DerefMut for CanvasRenderer<'a, ID> {
    #[inline(always)]
    fn deref_mut(&mut self) -> &mut Canvas<ID> {
        &mut self.canvas
    }
}
