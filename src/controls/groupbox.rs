...
impl<S: Clone + Into<String>, ID: Hash + Clone> ControlT<ID> for GroupBoxT<S, ID> {
    ...
    fn build(&self, ui: &Ui<ID>) -> Result<Box<Control>, Error> {
        ...
        use winapi::{
            BS_CENTER, BS_GROUPBOX, BS_LEFT, BS_NOTIFY, BS_RIGHT, BS_TOP, DWORD, WS_CHILD,
        ...
        let parent = match handle_of_window(
            ui,
            &self.parent,
            "The parent of a groupbox must be a window-like control.",
        ) {
            Ok(h) => h,
            Err(e) => {
                return Err(e);
        ...
        let font_handle: Option<HFONT> = match self.font.as_ref() {
            Some(font_id) => match handle_of_font(
                ui,
                &font_id,
                "The font of a groupbox must be a font resource.",
            ) {
                Ok(h) => Some(h),
                Err(e) => {
                    return Err(e);
