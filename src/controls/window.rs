...
impl Window {
    ...
    */
    ...
    pub fn set_title<'a>(&self, text: &'a str) {
        unsafe {
...
unsafe extern "system" fn window_sysproc(hwnd: HWND, msg: UINT, w: WPARAM, l: LPARAM) -> LRESULT {
    ...
    use winapi::{GWL_USERDATA, WM_CLOSE, WM_CREATE};
...
unsafe fn build_window<S: Clone + Into<String>>(t: &WindowT<S>) -> Result<HWND, Error> {
    use low::window_helper::{build_window, set_window_long, WindowParams};
    use winapi::{
        DWORD, GWL_USERDATA, WS_CAPTION, WS_CLIPCHILDREN, WS_DISABLED, WS_MAXIMIZEBOX,
        WS_MINIMIZEBOX, WS_OVERLAPPED, WS_OVERLAPPEDWINDOW, WS_SYSMENU, WS_VISIBLE,
    };

    let fixed_window: DWORD =
        WS_CLIPCHILDREN | WS_SYSMENU | WS_CAPTION | WS_OVERLAPPED | WS_MINIMIZEBOX | WS_MAXIMIZEBOX;
    let flags: DWORD = if t.visible { WS_VISIBLE } else { 0 }
