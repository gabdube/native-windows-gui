...
impl<S1: Clone + Into<String>, S2: Clone + Into<String>, ID: Hash + Clone> ControlT<ID>
    for TextInputT<S1, S2, ID>
{
    ...
    fn build(&self, ui: &Ui<ID>) -> Result<Box<Control>, Error> {
        use low::defs::{EM_LIMITTEXT, ES_AUTOHSCROLL, ES_PASSWORD, ES_READONLY};
        use low::window_helper::{
            build_window, handle_of_font, handle_of_window, set_window_font, WindowParams,
        };
        use winapi::{DWORD, WS_BORDER, WS_CHILD, WS_DISABLED, WS_VISIBLE};
        ...
        let parent = match handle_of_window(
            ui,
            &self.parent,
            "The parent of a textinput must be a window-like control.",
        ) {
            Ok(h) => h,
            Err(e) => {
                return Err(e);
        ...
        let font_handle: Option<HFONT> = match self.font.as_ref() {
            Some(font_id) => match handle_of_font(
                ui,
                &font_id,
                "The font of a button must be a font resource.",
            ) {
                Ok(h) => Some(h),
                Err(e) => {
                    return Err(e);
...
impl TextInput {
    ...
    pub fn set_limit(&self, limit: u32) {
        ...
        unsafe {
            SendMessageW(self.handle, EM_LIMITTEXT, limit as WPARAM, 0);
        }
    }
...
fn set_placeholder<S: Into<String>>(handle: HWND, placeholder: S) {
    ...
    unsafe {
        SendMessageW(handle, EM_SETCUEBANNER, 0, mem::transmute(text.as_ptr()));
    }
}
