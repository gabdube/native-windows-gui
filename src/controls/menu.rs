...
impl Menu {
    ...
    pub fn set_enabled(&self, enabled: bool) {
        unsafe {
            ::low::menu_helper::enable_menu(self.handle, &self.parent, enabled);
        }
    }
}
...
impl<S: Clone + Into<String>, ID: Hash + Clone> ControlT<ID> for MenuItemT<S, ID> {
    ...
    fn build(&self, ui: &Ui<ID>) -> Result<Box<Control>, Error> {
        ...
        match handle_result {
            Ok((parent, uid)) => Ok(Box::new(MenuItem {
                parent: parent,
                unique_id: uid,
            }) as Box<Control>),
            Err(e) => Err(e),
...
impl MenuItem {
    ...
    pub fn set_enabled(&self, enabled: bool) {
        unsafe {
            ::low::menu_helper::enable_menuitem(self.parent, None, Some(self.unique_id), enabled);
        }
    }
}
...
impl<ID: Hash + Clone> ControlT<ID> for SeparatorT<ID> {
    ...
    fn build(&self, ui: &Ui<ID>) -> Result<Box<Control>, Error> {
        ...
        match handle_result {
            Ok((parent, uid)) => Ok(Box::new(Separator {
                parent: parent,
                unique_id: uid,
            }) as Box<Control>),
            Err(e) => Err(e),
...
#[inline(always)]
unsafe fn build_menu<S: Clone + Into<String>, ID: Clone + Hash>(
    ui: &Ui<ID>,
    t: &MenuT<S, ID>,
) -> Result<(HMENU, AnyHandle), Error> {
    use low::menu_helper::{enable_menuitem, use_menu_command};
    use low::other_helper::to_utf16;
    use user32::{AppendMenuW, CreateMenu, DrawMenuBar, GetMenu, SetMenu};
    use winapi::{MF_POPUP, MF_STRING};

    let ph_result = ui.handle_of(&t.parent);
    if ph_result.is_err() {
        return Err(ph_result.err().unwrap());
    }
    ...
    match ph_result.unwrap() {
        ...
        AnyHandle::HMENU_ITEM(_, _) => Err(Error::BadParent(
            "Window or menu parent required, got MenuItem".to_string(),
        )),
        AnyHandle::HFONT(_) => Err(Error::BadParent(
            "Window or menu parent required, got Font".to_string(),
        )),
        AnyHandle::Custom(_, _) => Err(Error::BadParent(
            "Window or menu parent required, got custom control".to_string(),
        )),
    }
}

#[inline(always)]
unsafe fn build_menu_item<S: Clone + Into<String>, ID: Clone + Hash>(
    ui: &Ui<ID>,
    t: &MenuItemT<S, ID>,
) -> Result<(HMENU, UINT), Error> {
    ...
    match ph_result.unwrap() {
        AnyHandle::HWND(parent_h) => {
            ...
            AppendMenuW(
                menubar,
                MF_STRING,
                ensure_id_stays_the_same as UINT_PTR,
                text.as_ptr(),
            );
            enable_menuitem(menubar, None, None, !t.disabled);
        ...
        AnyHandle::HMENU(parent_h) => {
            ...
            AppendMenuW(
                parent_h,
                MF_STRING,
                MENU_ITEMS_ID as UINT_PTR,
                text.as_ptr(),
            );
            enable_menuitem(parent_h, None, None, !t.disabled);
            Ok((parent_h, MENU_ITEMS_ID))
        }
        h => Err(Error::BadParent(format!(
            "A menu item parent must be a Menu or a Window. Got {:?}",
            h
        ))),
    }
}
...
#[inline(always)]
unsafe fn build_separator<ID: Clone + Hash>(
    ui: &Ui<ID>,
    t: &SeparatorT<ID>,
) -> Result<(HMENU, UINT), Error> {
    use low::defs::{GetMenuItemCount, SetMenuItemInfoW, MF_SEPARATOR, MIIM_ID};
    use std::ptr;
    ...
    match ph_result.unwrap() {
        ...
        h => Err(Error::BadParent(format!(
            "A separator parent must be a Menu. Got {:?}",
            h
        ))),
    }
}
